        ...
        for concept, body in concepts.items():
            ...
            for unit, observations in body.get("units", {}).items():
                ...
                try:
                    ...
                    continue
                if abs(value).bit_length() > 256 or scale > 30:
                    continue  # corrupt filing value; a real financial number never needs this
                fs.add(
                    FinFact(
...
def main() -> None:
    ...
    t0 = time.time()
    companies = facts = skipped = errors = 0
    for cik, doc in iter_bulk(args.zip_path):
        ...
        try:
            ...
            if canonical.cid(pub.kw.entity_record(entity)) in known:
                companies += 1  # published in a previous (idempotent) run
                continue
            ...
            errors += 1
            print(f"ERROR CIK{cik} {ticker}: {exc!r}", flush=True)
            continue
        if time.time() - tc > 10:
            print(f"SLOW CIK{cik} {ticker}: {time.time()-tc:.0f}s, {len(fs.facts)} facts", flush=True)
        if companies % 1000 == 0:
    ...
    head = pub.commit()
    print(f"DONE {companies} companies, {facts} records, {skipped} skipped, {errors} errors, {time.time()-t0:.0f}s")
