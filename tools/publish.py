...
class Publisher:
    ...
    def feed(self):
        ...
        Loads validated entries directly instead of Feed.append: append mints
        and signs a head per entry, which is O(n²) over a large feed
        (Knitweb/pulse#338). Entries are still canonical-checked one by one;
        the single head() at the end commits to the same state.
        """
        from knitweb.core import canonical
        ...
        for rec in self.store.iter_records():
            canonical.encode(rec)  # same validation Feed.append performs
            f._entries.append(rec)
        return f
